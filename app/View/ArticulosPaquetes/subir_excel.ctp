					...
					<ol>
						<li>Descargar el archivo Excel.</li>
							<div style="padding: 0px 0px 30px 0px;" class="input-field">
								<a href="/articulos_paquetes/descargar_excel/plantillaPaquetesDetalle" class="waves-effect waves-light btn">Descargar</a>
							</br>
							</div>
...
$("#menu_paqu").addClass("activado");
	...
	$('#ArticulosPaqueteSubirExcelForm').validate({
		rules: {
			'data[archivo]': {
				required: true
			}
	...
	$('#ArticulosPaqueteSubirExcelForm').submit(function(event)
	{
		if ($('#ArticulosPaqueteSubirExcelForm').valid())
		{
